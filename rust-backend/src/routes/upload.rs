    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| {
        ...
        if field.name() == Some("file") {
            ...
            if !allowed_types.contains(&content_type.as_str()) {
                return Err(AppError::Upload("Invalid file type. Allowed: images, videos, PDFs, documents.".to_string()));
            }
            ...
            if data.len() > state.config.max_file_size {
                return Err(AppError::Upload("File too large. Maximum size is 1GB.".to_string()));
            }
            ...
            fs::create_dir_all(&state.config.upload_dir).await.map_err(|e| {
            ...
            tracing::info!("File uploaded by user {}: {}", auth.user_id, unique_filename);
